...
for i in range(98) :
    time.sleep(10)
    clockin_cli = "ldconsole.exe action --index 0 --key call.locate --value " + clockin_location[str(clockin_id)]
    os.popen(clockin_cli)
    time.sleep(10)
    os.popen('ldconsole.exe adb --index 0 --command "shell input tap 57 106"')
    time.sleep(10)
    clockin_cli = 'ldconsole.exe adb --index 0 --command "shell screencap -p /sdcard/clockin/' + str(clockin_id) + '.png"'
    os.popen(clockin_cli)
    print ("ID" + str(clockin_id) + "的坐标已完成截图")
    clockin_id += 1
