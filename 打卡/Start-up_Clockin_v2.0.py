...
def clockin_check() :
    ...
    time.sleep(10)
    os.popen('ldconsole.exe adb --index 0 --command "pull /sdcard/clockin/Check.png ./clockin/Check.png"')
    time.sleep(10)
    reader = easyocr.Reader(['ch_sim'])
    result = reader.readtext('./clockin/Check.png')
    for i in result:
        word = i[1]
        match_fail = re.match(r'.*(加载中).*',word,re.I)
        if match_fail != None :
            print("检查成功")
            return 1
    print("检查成功")
...
def clockin_action() :
    ...
    time.sleep(10)
    Send_URL = urlopen("https://sctapi.ftqq.com/SCT148937TjbF0erjEQKSScMXqQljuqpah.send?title=Clock-In%20Complete%20" + position_id)
    time.sleep(60)
