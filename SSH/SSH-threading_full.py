...
def gather_ssh(name, ip, cli):
    ...
    try :
        ssh.connect(hostname = ip,port = 22,username = "admin",password = "Admin@123",timeout=180)
    except :
    ...
    while True :
        try :
            match_Obj_1 = re.match(r"(.+?),(\d+)",next(iter_cli),re.I)
    ...
    with open("[" + name + "]_[" + ip + "]" + ".txt","ab") as file :
        file.write(re.sub(b"(\r)|(\x00)",b"",re.sub(b"(\n.+More.+\x08)|(\n.+More.+\[16D)|(\n.+More.+\[42D)|(\n.+More.+\[m)|(\n.+More.+\r\r +\r)",b"\n",output)))
    time.sleep(1)
    print("完成采集_" + name)
    return
...
with open(file_cisco_cli,"r",encoding="UTF-8") as file_2 :
    for line_2 in file_2 :
        dict_cli["cisco"].append(re.sub(r"(\r\n)|(\n)","",line_2))
